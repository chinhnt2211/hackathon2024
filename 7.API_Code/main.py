...
app.add_middleware(
    CORSMiddleware,
    # Hoặc bạn có thể chỉ định các domain cụ thể thay vì cho phép tất cả
    allow_origins=["*"],
    allow_credentials=True,
    # Bạn có thể giới hạn các method được phép, ví dụ: ["GET", "POST"]
    allow_methods=["*"],
...
class webhook_status(BaseModel):
    status: str
    cluster_id: str
...
async def migrate(item: fdemigrate):
    ...
    response_copy_workflow = requests.post(
        url_copy_workflow, headers=headers, json=data_copy_workflow)
    ...
    url_launch_job = f'https://awx-dev.fde.fptcloud.com/api/v2/workflow_job_templates/{
        new_workflow_id}/launch/'
    data_launch_job = {
    ...
    response_launch_job = requests.post(
        url_launch_job, headers=headers, json=data_launch_job)
    ...
    cursor.execute(insert_query, (cluster_id, host, port,
                   username, password, status, workflow_id, migrate_at))
    connection.commit()
    cursor.close()
    connection.close()
...
async def migrate(cluster_id: str):
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Can not connect to database")


@app.post("/migration/webhook/callback-status")
async def callback_status(item: webhook_status):
    try:
        connection = psycopg2.connect(
            database="DBdefault",
            user="admin",
        ...
        WHERE cluster_id = %s;
        """)
        cursor.execute(query, (item.status, item.cluster_id))
        connection.commit()
        cursor.close()
        connection.close()
        return {
